...
def chat_ai():
    try:
        ...
        history_data = data.get('history', []) # Получаем историю из фронтенда
        
        # Форматируем историю для контекста ИИ
        history_str = ""
        if history_data:
            for item in history_data[:40]: # Берем последние 40 записей
        ...
        ТВОЯ ЗАДАЧА:
        1. Если пользователь хочет ЗАПИСАТЬ (есть сумма и категория):
           Верни СТРОГО JSON: {{"action": "add", "amount": число, "category": "название", "type": "inc/exp", "note": "описание"}}
        
        2. Если пользователь спрашивает АНАЛИЗ, СОВЕТ или ВОПРОС (сумма для записи отсутствует):
           Верни СТРОГО JSON: {{"action": "chat", "text": "Твой ответ на основе истории операций"}}
        ...
        # Убираем лишние символы, если ИИ прислал их вне JSON
